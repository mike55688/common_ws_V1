...
int main(int argc, char **argv)
{
    ...
    while (ros::ok()) // pub odom && cmd
    {
        ...
        if (calibration_flag == 999)
        {
            ...
            {
                send_data(Flag_start, 0, 0, wheel_angle - delt_ang+1.7, motor_Fork, 0, 0, 0, 0, 0, 0, 0); //电机(启动/停止)(1/0)，前轮速度 m/s，0，前轮转角 °/s . 起重电机PWM，范围-3600 ~ +3600 （PWM值）
            }
            else
            {
                send_data(Flag_start, wheel_speed, 0, wheel_angle - delt_ang+1.7, motor_Fork, 0, 0, 0, 0, 0, 0, 0); //电机(启动/停止)(1/0)，前轮速度 m/s，0，前轮转角 °/s . 起重电机PWM，范围-3600 ~ +3600 （PWM值）
            }
        }
