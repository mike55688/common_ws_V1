...
class Action():
    ...
    def fork_updown(self, desired_updownposition):#0~2.7
        ...
        if self.updownposition < desired_updownposition - self.fork_threshold:
            print('My 1111value is: %f     <    %f', self.updownposition , desired_updownposition - self.fork_threshold)    #檢查用
            ...
            print('My 2222value is: %f     >    %f', self.updownposition , desired_updownposition + self.fork_threshold)    #檢查用
            self.fork_msg.fork_velocity = -2000.0
            self.pub_fork.publish(self.fork_msg)
            return False
        else:
    ...
    def fnSeqChangingtheta(self, threshod):
        ...
        print('value is: %f     <    %f', abs(desired_angle_turn) , threshod)    #檢查用
        ...
        if self.current_nearby_sequence == self.NearbySequence.initial_turn.value:
            ...
            if abs(desired_angle_turn) < 0.02:
                ...
                if self.check_wait_time >8:
                    ...
                    self.check_wait_time =self.check_wait_time +1
            elif abs(desired_angle_turn) < 0.025 and self.check_wait_time :
                self.cmd_vel.fnStop()
                if self.check_wait_time > 8:
                    self.check_wait_time = 0
                    self.current_nearby_sequence = self.NearbySequence.go_straight.value
    ...
    def fnSeqParking(self, parking_dist):
        ...
        rospy.loginfo('My marker_2d_pose_x value is: %f', self.marker_2d_pose_x)
    ...
    def fnseqdead_reckoning(self, dead_reckoning_dist):#(使用里程紀計算)移動到離現在位置dead_reckoning_dist公尺的地方
        ...
        rospy.loginfo('My value is: %f', dist)
...
class cmd_vel():
    ...
    def fnTrackMarker(self, theta):
        Kp = 2.0 #6.5   #pid控制器的Kp  解決靠近tag什的不正常輪胎90度轉向

        twist = Twist()
        twist.linear.x = 0.05   #0.05
        twist.linear.y = 0
        twist.linear.z = 0
        twist.angular.x = 0
        twist.angular.y = 0
